    ...
    try {
        ...
        if(existingUser) {
            ...
            if (isPasswordValid) {
                ...
                return res.status(200).json({ message: 'Login successful',id:existingUser.id,Role:existingUser.Role});
            } else {
...
app.put('/enseignants/confirm/:Username', async (req, res) => {
    ...
    console.log('Request body:', req.params);
        ...
        return res.status(200).json({ message: 'Enseignant confirmed successfully' });
    } catch (error) {
...
app.delete('/super_users/:id', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Erreur lors de la suppression de super_user' });
    }
});
...
app.get('/super_users/:id', async (req, res) => {
    ...
    try {
        ...
        const super_user = await db.Super_user.findOne({ where: { id: super_userId } });
...
app.put('/super_users/edit/:id', async (req, res) => {
    ...
    try {
        ...
        if (updatedData.Role === 'Vice doyen') {
            ...
            if (!existingTeacher) {
                return res.status(400).json({ error: 'Super user is not an enseignant' });
            }
        } else if (updatedData.Role === 'Secrétaire' || updatedData.Role === 'Admin') {
            // Vérifier si un super user existe déjà avec le même username ou email
            const existingSuperUserWithSameUsernameOrEmail = await db.Super_user.findOne({
                where: {
                    [Op.and]: [
    ...
    try {
        if (userData.Role === 'Vice doyen') {
            ...
            if (!existingTeacher) {
                ...
                return res.status(400).json({ error: 'Vice dean does not exist in the teacher table' });
            }
            ...
            if (userData.Role === "Enseignant") {
                ...
                if (enseignant) {
                    ...
                    return res.status(201).json(newUser);
                        ...
                        return res.status(400).json({ error: 'Username not found in Enseignant et Doctorant table' });
                    }
                }
                ...
                if (doctorant) {
                    ...
                    return res.status(201).json(newUser);
                        ...
                        return res.status(400).json({ error: 'Username not found in Doctorant or Enseignant table' });
                    }
                }
            } else {
...
app.post('/membre_commission', async (req, res) => {
    ...
    try {
        ...
        if (existingMember) {
            return res.status(400).json({ error: 'Enseignant already exists in commission' });
        }
        // Ajouter l'enseignant à la commission
        await db.Membre_Commission.create({ Username_NSS });
        return res.status(201).json({ message: 'Enseignant added to commission successfully' });
    } catch (error) {
        console.error('Error adding enseignant to commission:', error);
        return res.status(500).json({ error: 'Failed to add enseignant to commission' });
    }
});
...
app.delete('/membre_commission/:Username_NSS', async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ message: 'Enseignant removed from commission successfully' });
    } catch (error) {
        console.error('Error removing enseignant from commission:', error);
        return res.status(500).json({ error: 'Failed to remove enseignant from commission' });
    }
});
//////////////////////////////////////////////////////////GET Donnéé MEMBRE/////////////////////////////////////////////////////////////////////////////////////////////
...
app.get('/membre_commission/info', async (req, res) => {
    try {
        // Récupérer les données des membres de la commission depuis la base de données
        const membresCommission = await Membre_Commission.findAll({ where: { Président: 'false' } });

        const membresPresident = await Membre_Commission.findAll({ where: { Président: 'true' } });
        ...
        const president = membresPresident.map(president => president.Username_NSS);
...
// Route pour supprimer un enseignant de la commission
app.delete('/membre_commission/:Username_NSS/president/delete', async (req, res) => {
    const { Username_NSS } = req.params;
    try {
        // Supprimer l'enseignant de la commission
        await db.Membre_Commission.destroy({
...
app.delete('/membre_commission/all', async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: 'All members removed from commission successfully' });
    } catch (error) {
        console.error('Error removing all members from commission:', error);
        return res.status(500).json({ error: 'Failed to remove all members from commission' });
    }
});
...
app.post('/demande_SPE', async (req, res) => {
    ...
    const { Username_Mat, Pays, Ville, Etablissement_acc, Periode_Stage, Annee, Date_dep, Date_retour } = req.body;
    ...
    try {
        ...
        if (existingDemande) {
            return res.status(400).json({ error: 'Demande already exists in SPE table' });
        }
        ...
        return res.status(201).json({ message: 'Demande ajoutée avec succès dans la table SPE' });
    } catch (error) {
        console.error('Erreur lors de l\'ajout de la demande dans la table SPE :', error);
        return res.status(500).json({ error: 'Échec de l\'ajout de la demande dans la table SPE' });
    }
});

  ...
  app.post('/binome_comission/create', async (req, res) => {
    try {
        ...
        for (let i = 0; i < enseignants.length; i += 2) {
            ...
            const typeTraitement = enseignantsTraites < halfLength ? 'traitement_enseignant' : 'traitement_doctorant';
        ...
        // Gérer les erreurs
        console.error('Erreur lors de la création des binômes de commission:', error);
        return res.status(500).json({ error: 'Échec de la création des binômes de commission' });
    }
});
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////Affichage BINOME///////////////////////////////////////////////////////////////////////////////////
...
app.get('/binome_comission', async (req, res) => {
    try {
        ...
        for (const binome of binomes) {
            ...
            const enseignant1 = await db.Enseignants.findOne({ where: { Username_NSS: binome.Username_Nss1 } });

            // Charger les informations de l'enseignant 2 associé au binôme
            const enseignant2 = await db.Enseignants.findOne({ where: { Username_NSS: binome.Username_Nss2 } });
